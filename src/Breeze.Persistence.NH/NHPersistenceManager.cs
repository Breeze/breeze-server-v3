...
namespace Breeze.Persistence.NH {
  ...
  public class NHPersistenceManager : Breeze.Persistence.PersistenceManager, IDisposable {
    ...
    protected NHBreezeMetadata GetMetadata() {
      ...
      // Set just above, either from the cache or by the builder.
      return _metadata!;
    }
    ...
    // Null until the first GetMetadata call, which caches the model per session factory.
    ...
    protected override void SaveChangesCore(SaveWorkState saveWorkState) {
      ...
      // tx is set either way now: the session already had one, or the branch above began one.
      try {
        ...
        // NHibernate raises this when a flush updates or deletes no rows, which is the optimistic
        // concurrency conflict. StaleObjectStateException, the usual subclass, also says which
        // entity; the plain StaleStateException does not.
        if (!hasExistingTransaction) tx!.Rollback();
        throw CreateConcurrencyErrorsException(sse);
      } catch (Exception) {
        if (!hasExistingTransaction) tx!.Rollback();
        throw;
      } finally {
    ...
    /// <summary>
    /// Turn NHibernate's stale-state exception into Breeze's, so a concurrency conflict reaches
    /// the client as 409 with a stable problem type rather than a 500 carrying NHibernate's
    /// message text - the same document an EF Core conflict produces.
    /// </summary>
    /// <param name="sse">The exception NHibernate raised when a flush matched no rows.</param>
    /// <returns>An exception naming the entity that conflicted, where NHibernate says which.</returns>
    private static ConcurrencyErrorsException CreateConcurrencyErrorsException(StaleStateException sse) {
      var entityErrors = new List<EntityError>();
      // Only StaleObjectStateException carries the entity name and identifier. The base class is
      // raised for a batched flush, where NHibernate knows a row was stale but not which one, so
      // the client still gets the status and the problem type but no per-entity error.
      if (sse is StaleObjectStateException sose) {
        entityErrors.Add(ConcurrencyErrorsException.CreateEntityError(
          sose.EntityName,
          sose.Identifier == null ? null : new object[] { sose.Identifier }));
      }
    ...
    protected void AddKeyMapping(EntityInfo entityInfo, Type type, IClassMetadata meta) {
      ...
      var km = new KeyMapping() { EntityTypeName = type.FullName!, TempValue = id };   // a mapped class has a FullName
