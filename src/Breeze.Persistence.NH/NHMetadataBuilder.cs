...
namespace Breeze.Persistence.NH {
  ...
  public class NHMetadataBuilder {
    ...
    // InitMap replaces all five at the start of every build; the initializers are what let the
    // compiler see them as non-null.
    private NHBreezeMetadata _map = new NHBreezeMetadata();
    private List<MetaType> _typeList = new List<MetaType>();
    ...
    void AddClass(IClassMetadata meta) {
      ...
      // BuildMetadata only ever passes persisters, and the metamodel is read unconditionally below.
      var entityPersister = (IEntityPersister)meta;
      ...
      var generator = entityPersister.IdentifierGenerator;
      if (generator != null) {
    ...
    void AddClassProperties(IClassMetadata meta, List<MetaDataProperty> dataList, List<MetaNavProperty> navList) {
      // Every mapped class metadata is an AbstractEntityPersister; anything else failed here already.
      var persister = (AbstractEntityPersister)meta;
      ...
      for (int i = 0; i < propNames.Length; i++) {
        ...
        if (propType is AbstractEnumType) {
          ...
          if (types.Length > 0) {
            ...
            // Null for an enum whose underlying type is not int - as it has always been here.
        ...
        if (fkNames != null) {
          ...
          if (isKey) {
            foreach (var fkName in fkNames) {
              // fkName was just looked up from these very data properties.
              var relatedDataProperty = (MetaDataProperty)FindPropertyByName(dataProperties, fkName)!;
    ...
    static Type GetEntityType(Type type, bool isCollectionType) {
      if (!isCollectionType) {
        ...
        return type.GetElementType()!;   // non-null when HasElementType
  ...
  public class NHBreezeMetadata : BreezeMetadata {
    ...
    /// </summary>
    [JsonIgnore]
    public IDictionary<string, string> ForeignKeyMap = new Dictionary<string, string>();
