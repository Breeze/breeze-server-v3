...
namespace Breeze.Persistence.EFCore {
  ...
  public class EFPersistenceManager<T> : PersistenceManager, IEFContextProvider where T : DbContext {
    ...
    // Null only for a manager built with the parameterless constructor. Such a manager has
    // no context, and every member that needs one fails (as it always has) - hence the '!'s
    // in DbContext and Context.
    private T? _context;
    ...
    protected override IDbTransaction BeginTransaction(System.Data.IsolationLevel isolationLevel) {
      ...
      if (conn == null) return null!; // unreachable: EF's GetDbConnection() never returns null
    ...
    /// <summary> Allow a subclass to provide alternate metadata; null (the default) for none </summary>
    ...
    public object[] GetKeyValues(object entity) {
      ...
      // EF does not allow null primary key values on an entity it saves.
    ...
    private IEnumerable<PropertyInfo> GetKeyProperties(Type entityType) {
      // Entities being saved are of mapped entity types with a primary key (EF could not
      // save them otherwise), and Breeze does not support shadow key properties; anything
      // else fails here, as it always has.
    ...
    private void ProcessAllDeleted(List<EFEntityInfo> deletedEntities) {
      deletedEntities.ForEach(entityInfo => {
        ...
        entry.State = Microsoft.EntityFrameworkCore.EntityState.Deleted;
          ...
          // A null owned (complex-type) value fails in DbContext.Entry, as it always has.
    ...
    private void ProcessAutogeneratedKeys(List<EntityInfo> entitiesWithAutoGeneratedKeys) {
      // EntitiesWithAutoGeneratedKeys only holds entities whose AutoGeneratedKey is set.
    ...
    private IKeyGenerator GetKeyGenerator() {
      ...
      // Creating a class instance never yields null.
    ...
    private List<KeyMapping> UpdateAutoGeneratedKeys(List<EntityInfo> entitiesWithAutoGeneratedKeys) {
      ...
      var keyMappings = entitiesWithAutoGeneratedKeys.Cast<EFEntityInfo>()
        ...
        .Select((Func<EFEntityInfo, KeyMapping>)(entityInfo => {
          // Every entity here has an AutoGeneratedKey (see MakeAutoKeys) and, per the
          // Where above, an EntityEntry; an object's runtime type always has a FullName.
    ...
    private static Object? ConvertValue(Object? val, Type toType) {
      ...
      if (nnToType.IsEnum && val is string) {
        result = Enum.Parse(nnToType, (string)val, true);
  ...
  public class EFEntityInfo : EntityInfo {
    ...
    // Both set while the save is processed; EntityEntry stays null for deleted entities
    // until ProcessAllDeleted runs.
