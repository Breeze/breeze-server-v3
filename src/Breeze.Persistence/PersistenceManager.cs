...
namespace Breeze.Persistence {
  ...
  public class SaveWorkState {
    ...
    /// <summary> The manager running this save. </summary>
    public PersistenceManager PersistenceManager;
    /// <summary> The entities from the save bundle, grouped by type, before the before-save hooks filter them. </summary>
    ...
    /// <summary> The entities to be saved, by type.  Set by <see cref="BeforeSave"/>; the before-save hooks may add to, change or remove from it. </summary>
    public Dictionary<Type, List<EntityInfo>> SaveMap { get; set; } = null!;
    /// <summary> The entities in <see cref="SaveMap"/> that need a server-generated key. </summary>
    public List<EntityInfo> EntitiesWithAutoGeneratedKeys { get; set; } = null!;
    /// <summary> Temporary keys from the client mapped to the real keys, filled in by the save and returned to the client. </summary>
    public List<KeyMapping> KeyMappings = null!;
    /// <summary> Errors that stopped the save, or null if it did not fail validation. </summary>
    public List<EntityError>? EntityErrors;
    /// <summary> Whether this state has already produced a save result.  A used state is rebuilt rather than saved again. </summary>
    public bool WasUsed { get; internal set; }

    /// <summary> The entities of one type, as they arrived in the save bundle. </summary>
    public class EntityGroup {
      ...
      /// <summary> The entities of that type, with the state each is to be saved in. </summary>
  ...
  public class EntityKey {
    ...
    /// <summary> Create an empty key.  Exists for deserialization, which fills in both members. </summary>
    public EntityKey() { }
    /// <summary> Identify an entity by its type and key value. </summary>
    /// <param name="entity">The entity, from which the type name is taken.</param>
    /// <param name="key">The key value: a single value, or an array for a composite key.</param>
  ...
  public class JsonPropertyFixupWriter : JsonTextWriter {
    /// <summary> Create a writer over a text writer. </summary>
    /// <param name="textWriter">Receives the JSON.</param>
    public JsonPropertyFixupWriter(TextWriter textWriter)
      : base(textWriter) {
      _isDataType = false;
    }

    /// <summary> Write a property name, dropping the leading "@" that XML attributes turn into and lower-casing the first letter. </summary>
    /// <param name="name">The property name as it came from the mapping XML.</param>
    ...
    /// <summary> Write a string value, prefixing "Edm." onto the value of a <c>type</c> property that lacks it. </summary>
